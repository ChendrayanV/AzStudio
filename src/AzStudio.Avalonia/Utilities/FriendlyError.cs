...
public static class FriendlyError
{
    public static string Summarize(Exception ex, string resourceLabel)
    {
        ...
        if (ex is RequestFailedException rfe)
        {
            ...
            if (rfe.ErrorCode == "InsufficientAccessError")
            {
                return $"Access denied to '{resourceLabel}'. Same pattern again — management-plane roles don't grant " +
                       "query access to a workspace's log data. Ask an admin for the 'Log Analytics Reader' role on " +
