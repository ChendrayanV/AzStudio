...
/// <summary>
/// Port of AzStudio.App's BlobStorageTabViewModel for the Avalonia spike. Business logic
/// (everything not touching a dialog/clipboard/file-picker) is byte-for-byte identical to
/// the WPF version — the only changes are the four call sites that touched WPF-specific
/// APIs (Clipboard, MessageBox, Microsoft.Win32 file dialogs, Application.Current.MainWindow),
/// now routed through AvaloniaDialogs and made async to match Avalonia's dialog APIs.
/// </summary>
public partial class BlobStorageTabViewModel : ObservableObject
{
    ...
    private async Task CreateContainerAsync()
    {
        ...
        var name = await AvaloniaDialogs.PromptTextAsync("New Container", "Container name (lowercase, digits, hyphens):");
    ...
    {
        if (_service is null || SelectedContainer is null) return;
        var confirmed = await AvaloniaDialogs.ConfirmAsync(
    ...
    {
        if (_service is null || SelectedContainer is null) return;
        var path = await AvaloniaDialogs.OpenFileAsync("Select file to upload");
        ...
        {
            SetError("Upload blob", ex, $"blob '{Path.GetFileName(path)}'");
        }
        finally
        {
            IsBusy = false;
    ...
    private async Task DownloadAsync()
    {
        ...
        var path = await AvaloniaDialogs.SaveFileAsync("Save blob as", SelectedBlob.Name);
        if (path is null) return;
        ...
        try
        {
            ...
            StatusMessage = $"Downloaded '{SelectedBlob.Name}' to {path}.";
            ClearError();
        }
        catch (Exception ex)
        {
    ...
    {
        if (_service is null || SelectedContainer is null || SelectedBlob is null) return;
        var confirmed = await AvaloniaDialogs.ConfirmAsync($"Delete blob '{SelectedBlob.Name}'?", "Confirm Delete");
        if (!confirmed) return;
